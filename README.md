...
`kcen-aoc` that, given a day and input, will
provide the solution.

```
kcen-aoc 3 my_input.txt
```
...
`kcen-aoc-bench` that does not rely on `click` but is less
...
```
# with plain poetry
poetry run kcen-aoc DAY PATH_TO_INPUT

# with poetry shell activated (or if you just installed the distribution)
kcen-aoc DAY PATH_TO_INPUT

# example
kcen-aoc 2 inputs/day02.txt
```
